...
const HEADER_ALIASES = {
  ...
  iphoneModel: ['รุ่น iPhone', 'รุ่นiphone', 'iphone รุ่น', 'iphone model', 'phone model', 'model iphone', 'Product', 'สินค้า', 'รุ่นสินค้า'],
  lotReceived: ['Lot รับของลูกค้าแล้ว', 'Lot รับของลูกค้า', 'Lot Date', 'lot date', 'customer lot', 'lot'],
...
export function mapSheetRows(matrix) {
  ...
  const indexes = {
    ...
    iphoneModel: findHeaderIndex(headers, HEADER_ALIASES.iphoneModel),
    lotReceived: findHeaderIndex(headers, HEADER_ALIASES.lotReceived),
  ...
  return matrix.slice(1).map((values) => ({
    ...
    iphoneModel: cleanCell(values[indexes.iphoneModel] ?? ''),
    lotReceived: cleanCell(values[indexes.lotReceived] ?? ''),
